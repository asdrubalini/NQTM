...
class TopicDisQuant(object):
    ...
    def quantize(self, encoding_indices):
        ...
        return tf.nn.embedding_lookup(self._E, encoding_indices)
...
class NQTM(object):
    ...
    def negative_sampling(self, theta):
        ...
        topic_indices = tf.one_hot(tf.random.categorical(logits, self.select_topic_num), depth=theta.shape[1])  # N*1*K
    ...
    def forward(self, x):
        ...
        if self.word_sample_size > 0:
            ...
            self.auto_encoding_error = tf.reduce_mean(tf.reduce_sum(-x * tf.math.log(self.x_recon) + negative_error, axis=1))
            self.loss = self.auto_encoding_error + quantization_output["loss"]
        else:
            self.auto_encoding_error = tf.reduce_mean(tf.reduce_sum(-x * tf.math.log(self.x_recon), axis=1))
            self.loss = self.auto_encoding_error + quantization_output['loss']
